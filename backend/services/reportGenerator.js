...
async function buildReport(dataCallback, endCallback, results, appUrl) {
    ...
    const totalScore = results.reduce((sum, r) => sum + r.screeningResult.match_score, 0);
    const averageScore = results.length > 0 ? Math.round(totalScore / results.length) : 'N/A';
    ...
    generateStatCard(doc, 50, 140, 'TOTAL RESUMES', `${results.length}`);
    generateStatCard(doc, 225, 140, 'AVERAGE SCORE', `${averageScore}/100`);
    generateStatCard(doc, 400, 140, 'TOP CANDIDATE', topCandidate, 18);
    ...
    if (results && results.length > 0) {
        ...
        const cleanedJd = results[0].jobDescription.replace(/\r/g, '');

        doc.fontSize(11).font('Inter-Regular').fillColor(colors.muted).text(cleanedJd, {
            align: 'justify',
            lineGap: 4
        });
        await generateFooter(doc, appUrl);
    ...
    for (const result of results) {
        ...
        if (criteria_scores) {
            doc.fontSize(12).font('Inter-Bold').fillColor(colors.text).text('Score Breakdown', leftColumnX, columnStartY);
            doc.y = columnStartY + 20;
            doc.x = leftColumnX;
            Object.entries(criteria_scores).forEach(([key, value]) => {
                generateScoreBar(doc, key, value);
            });
        }
